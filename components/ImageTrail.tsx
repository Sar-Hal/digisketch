...
class ImageTrailVariant1 {
  constructor(container: HTMLElement) {
    ...
    this.images = [...this.DOM.el.querySelectorAll(".image-trail__img")].map(
    ...
    const handlePointerMove = (ev: PointerEventLike) => {
    ...
    const initRender = (ev: PointerEventLike) => {
      const rect = this.container.getBoundingClientRect();
      this.mousePos = getLocalPointerPos(ev, rect);
...
class ImageTrailVariant2 {
  ...
  showNextImage() {
    ...
    if (!img.DOM.el || !img.rect || !img.DOM.inner) {
      return;
    }
...
export default function ImageTrail({
  ...
  return (
    <div className={`image-trail ${className}`} ref={containerRef}>
      {items.map((url, index) => (
